...
public class EmployeeService extends ServiceBase{
    ...
    /**
     * idを条件に取得したデータをEmployeeViewのインスタンスを返却する
     * @param id
     * @return 取得データのインスタンス
     */
    ...
    /**
     * 画面から入力された従業員の登録内容を元にデータを1件作成し、従業員テーブルを更新する
     * @param ev 画面から入力された従業員の登録内容
     * @param pepper pepper文字列
     ...
     * @param pepper pepper文字列
     * @return 認証結果を返却（true or false）
     */
    private Boolean validateLogin(String code, String plainPass, String pepper) {
    ...
    /**
     * idを条件にデータを1件取得し、Employeeのインスタンスで返却
     * @param id
     * @return 取得データのインスタンス
     */
    ...
    private void update(EmployeeView ev) {
        ...
        EmployeeConverter.copyViewsToModel(e, ev);
        em.getTransaction().commit();
    }
}
