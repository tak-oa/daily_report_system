...
public class ReportService extends ServiceBase {
    ...
    /**
     * 指定されたページ数の一覧画面に表示する未承認の日報データを取得し、ReportViewのリストで返す
     * @param page ページ数
     * @return 一覧画面に表示するデータのリスト
     */
    public List<ReportView> getUnappPerPage(int page) {

        List<Report> unapp_reports = em.createNamedQuery(JpaConst.Q_REP_APP_GET_ALL, Report.class)
                .setFirstResult(JpaConst.ROW_PER_PAGE * (page - 1))
                .setMaxResults(JpaConst.ROW_PER_PAGE)
                .getResultList();
        return ReportConverter.toViewList(unapp_reports);
    }
    /**
    ...
    public long countUnapproved() {
        long reports_unapproved_count = (long) em.createNamedQuery(JpaConst.Q_REP_UNAPPROVAL_COUNT, Long.class)
                .getSingleResult();
        return reports_unapproved_count;
    }
